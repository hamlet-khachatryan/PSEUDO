...
def find_covalent_neighbors_fast(
    ...
    Args:
        model: A gemmi.Model
        structure: The parent gemmi.Structure
        tolerance: Distance tolerance. Default 0.4 A
    Returns:
                    ...
                    if dist <= cr_i + cr_j + tolerance:
                        for ci2, chain2 in enumerate(model):
                            ...
                            for ri2, res2 in enumerate(chain2):
                                if (
                                    str(res2.seqid) != str(cra.residue.seqid)
                                    or res2.name != cra.residue.name
                                ):
                                    continue
                                for ai2, at2 in enumerate(res2):
                                    if (
                                        at2.name == neighbor_atom.name
                                        and not at2.element.is_hydrogen
                                    ):
                                        aid_j = (ci2, ri2, ai2)
                                        if (
                                            aid_j != aid_i
                                            and aid_j not in adjacency[aid_i]
                                        ):
                                            adjacency[aid_i].append(aid_j)
                                        break
                                break
